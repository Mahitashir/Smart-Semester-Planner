...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_num_rows($check) > 0) {
        $message = "<div class='msg error'>FAILED TO UPDATE. COURSE ALREADY EXISTS.</div>";
    } else {
        ...
        if (mysqli_query($conn, $insert_query)) {
            ...
            if (!empty($_POST['prerequisites'])) {
                foreach ($_POST['prerequisites'] as $prereq_id) {
                    if ($prereq_id !== 'none') {
                        mysqli_query($conn, "INSERT INTO PREREQUISITE (course_id, prereq_course_id) VALUES ('$new_course_id', '$prereq_id')");
                    }
                }
            }
            $message = "<div class='msg success'>Course successfully added to catalog!</div>";
        } else {
...
<head>
    ...
    <style>
        ...
        /* Full Width Top Banner (Matching Student) */
        .header-banner { 
        ...
        .prereq-box {
            height: 120px; overflow-y: auto; border: 1px solid #ccc; border-radius: 8px; padding: 12px; background: #f9f9f9;
        }
        .prereq-box label { display: block; cursor: pointer; margin-bottom: 8px; font-weight: normal; color: #333;}
        .prereq-box input[type="checkbox"] { margin-right: 10px; }
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group full-width">
                        ...
                        <div class="prereq-box">
                            <label><input type="checkbox" name="prerequisites[]" value="none" checked> None (No Prerequisite)</label>
                            <?php while ($row = mysqli_fetch_assoc($courses_result)) { ?>
                                <label>
                                    <input type="checkbox" name="prerequisites[]" value="<?php echo $row['course_id']; ?>"> 
